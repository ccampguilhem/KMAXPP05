...
\begin{frame}{Application de la règle de la chaîne}
	...
	\begin{itemize}
		\pause
		\item Les dérivées font apparaître des \textbf{tenseurs}.
		\pause
...
\begin{frame}{Dérivées des opérateurs}
	...
	On verra plus tard dans le cours comment on calcule la dérivée des fonctions de perte. Ici, pour rester le plus générique possible, on notera simplement $\delta_0$ la dérivée de l'erreur par rapport aux prédictions $\hat{Y}$:
	...
	Si l'erreur est un scalaire $e \in \R$, la dérivée de l'erreur est un vecteur colonne: $\delta_0 \in \mathcal{M}_{n,m}(\R)$ car nous avons empilé verticalement les $n$ observations dans le vecteur cible: $Y \in \mathcal{M}_{n,m}(\R)$. Donc les prédictions se retrouvent empilées de la même manière: $\hat{Y} \in \mathcal{M}_{n,m}(\R)$. Avec $m=1$ en régression et en classification binaire.
\end{frame}
...
\begin{frame}{Fonction logistique tensorielle}
	\begin{block}{Définition de la fonction logistique tensorielle}
		...
		\end{equation*}
		L'exponentielle est appliquée à chaque membre du tenseur $Z$. La sortie est un tenseur de même dimension que le tenseur d'entrée $Z$. $1_Z$ dans cette expression est en fait un tenseur de $1$ de même dimension que $Z$.
	\end{block}
	Exemple:
	\begin{equation*}
		Z = \begin{pmatrix}
...
\begin{frame}{Propagation d'un gradient par une fonction d'activation}
	\begin{block}{Propagation d'un gradient $\delta$ par une fonction d'activation}
		...
		\end{equation*}
	\end{block}
	Note: la démonstration est disponible dans les annexes de ce cours.
\end{frame}
...
\begin{frame}{Explication de la contraction tensorielle}
	Pour une fonction de plusieurs variables $f(x_1, x_2)$, la différentielle totale s'écrit:
	\begin{equation*}
		df = \frac{\partial{f}}{\partial{x_1}} dx_1 + \frac{\partial{f}}{\partial{x_2}} dx_2
	\end{equation*}
	\pause
	Cela consiste donc à \textbf{sommer} les contributions de chaque variable influençant le résultat. Le gradient que l'on propage est $\frac{\partial{e}}{\partial{V}}$ c'est un tenseur de la même dimension que $V$. Mais l'erreur $e$ est elle-même un scalaire. Donc, dans cet opérateur ce qui influence l'erreur est l'ensemble des composantes de $V$. Si par exemple on prend $n = 3$ et $v = 2$:
	\begin{equation*}
		\begin{aligned}
...
\begin{frame}{Explication de la contraction tensorielle}
	On peut simplifier cette somme en:
	\begin{equation*}
		de = \sum_{\mu=1}^n \sum_{\nu=1}^v \frac{\partial{e}}{\partial{V_{\mu,\nu}}} dV_{\mu,\nu}
	\end{equation*}
	\pause
	On peut maintenant faire apparaître la variation d'une quantité $Q$ quelconque:
	\begin{equation*}
		de = \sum_{\mu=1}^n \sum_{\nu=1}^v \frac{\partial{e}}{\partial{V_{\mu,\nu}}} \frac{\partial{V}_{\mu,\nu}}{\partial{Q}} dQ 
	\end{equation*}
	\pause
	Ce qui fait apparaître l'expression de la contraction tensorielle vue auparavant, considérant que $e$ dépend d'autres variables que $Q$:
	\begin{equation*}
...
\begin{frame}{Propagation d'un gradient par un opérateur linéaire}
	\begin{block}{Propagation d'un gradient $\delta$ par un opérateur linéaire}
		...
		\end{equation*} 
	\end{block}
	Note: les démonstrations sont disponibles dans les annexes de ce cours.
\end{frame}
...
\begin{frame}{Synthèse sur la rétro-propagation du gradient}
	...
	\begin{itemize}
		...
		\item Appliquer la \textbf{règle de la chaîne} pour déterminer analytiquement toutes les dérivées de l'\textbf{erreur} en fonction des différents \textbf{paramètres} du réseau (poids et biais).
