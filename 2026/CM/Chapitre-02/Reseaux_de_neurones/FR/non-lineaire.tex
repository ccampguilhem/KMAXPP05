...
\begin{frame}{Problèmes non-linéaires}
	...
	\begin{itemize}
		\item Comment adapter la définition d'un neurone artificiel pour le préparer à l'algorithme d'apprentissage de \textbf{rétro-propagation du gradient}.
		\item Comment créer un \textbf{réseau} pour résoudre un problème non-linéaire.
		\item Comment adapter la structure d'un neurone artificiel pour les problèmes de \textbf{régression}.
	\end{itemize}
\end{frame}
...
\begin{frame}{Un abus de langage usuel pour les réseaux de neurones}
	...
	Mais c'est un abus. En effet $X \in \mathcal{M}_{n,p}(\R)$, $\hat{Y} \in \mathcal{M}_{n,1}(\R)$, $w \in \mathcal{M}_{p,1}(\R)$ et $b \in \R$. On ne peut rigoureusement pas additionner un vecteur et un scalaire.
	
	\vspace{0.2cm}
	\pause
	
	C'est une pratique qui nous vient du monde de l'informatique. Les librairies de calcul utilisent un concept de \textbf{broadcasting} qui permet de modifier automatiquement la forme d'un opérande entre vecteurs, matrices, tenseurs et scalaires. Rigoureusement, l'opération qui va être réalisée est la suivante:
...
\begin{frame}{Le broadcasting pour une couche de neurones}
	...
	Dans cette expression, le produit $X^{[k]} W^{[k+1]}$ est une matrice de dimension $n \times p^{[k+1]}$ et $b^{[k+1]}$ est un vecteur ligne $1 \times p^{[k+1]}$. L'opération de broadcasting réalisée par un programme informatique est la suivante:
...
\begin{frame}{Choix des fonctions d'activation}
	...
	\begin{itemize}
		\item Le choix de la fonction d'activation sur la couche de sortie est généralement dicté par la nature du problème. Dans le cadre d'une \textbf{classification binaire}, la sortie $\hat{Y}$ représente des probabilités, utiliser une fonction \textbf{logistique} est alors naturel.
		\pause
		\item Pour une classification multi-classe, utiliser une fonction \textbf{softmax} en sortie est un choix très pertinent. 
		\pause
		\item Pour un problème de \textbf{régression}, il n'y a en général pas de fonction d'activation sur la couche de sortie. Un réseau à une couche de neurones avec un neurone est donc \textbf{strictement} équivalent à la formulation mathématique d'une \textbf{régression linéaire}: $\hat{Y} = XW + b$
		\pause
...
\begin{frame}{Choix de la fonction d'activation des couches cachées}
	...
	\begin{itemize}
		...
		\pause
		\item La fonction logistique et la tangente hyperbolique offrent de meilleures performances sur des réseaux de neurones simples. On limite en général la fonction logistique à la couche de sortie.
	\end{itemize}
\end{frame}
