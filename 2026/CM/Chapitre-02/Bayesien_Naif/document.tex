...
\begin{frame}{Algorithme Bayésien Naïf}
	...
	\begin{itemize}
		...
		\pause
		\item ...avec quelques ajustements qui lui valent son attribut de \textbf{naïf}.
	\end{itemize}
\end{frame}
...
\begin{frame}{Plan du cours}
	...
	\begin{itemize}
		...
		\item Traitement simples de données textuelles
	\end{itemize}
\end{frame}
...
\begin{frame}{Variable aléatoire discrète}
	...
	\end{equation}
	\pause
	Ou dans une forme plus générale si $A \in \{0, 1, \cdots, k\}$:
	\begin{equation}
		\begin{aligned}
...
\begin{frame}{Une critique plus élaborée}
	...
	\begin{itemize}
		\item Les mots \text{of} et \text{time} ne font pas partie du \textbf{vocabulaire}, ou du \textbf{corpus} que nous avons considéré.
		\pause
		...
		\pause
		\item Nous cherchons donc à calculer: $P(C = 1 \mid \text{"boring movie waste"})$ qui nécessite de connaitre de la vraisemblance $P(\text{"boring movie waste"} \mid C = 1)$.
		\pause
	...
	\end{itemize}
	\pause
	C'est là que la nature \textbf{naïve} de l'algorithme intervient. Pour simplifier, on va considérer que les mots au sein d'une même phrase sont indépendant. C'est \textbf{faux} mais cela simplifie grandement les calculs !
\end{frame}
...
\begin{frame}{Nature multinomiale de l'algorithme}
	\begin{itemize}
		...
		\pause
		\item On considère chaque mot comme indépendant. Donc Il s'agit de faire 3 mise à jours avec le théorème de Bayes, ou multiplier 3 facteurs de Bayes.
		\pause
...
\begin{frame}{Maximum a posteriori}
	...
	\end{block}
	\pause
	Note: L'hypothèse naïve \textit{biaise} la probabilité a posteriori, mais comme nous ne recherchons que le maximum a posteriori, l'impact de cette hypothèse simpliste est limitée sur les cas d'application.
\end{frame}
