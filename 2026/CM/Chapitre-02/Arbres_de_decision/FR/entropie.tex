...
\begin{frame}{Un exemple avec une pièce de monnaie}
	Prenons un exemple simple. Nous disposons d'une pièce de monnaie qui n'est pas équilibrée. Les probabilités d'obtenir "Face" ou "Pile" sur un lancer $X$ sont les suivantes:
	\begin{equation*}
		\begin{cases}
			P(X = \text{"Face"}) = 0 \\
			P(X = \text{"Pile"}) = 1
	...
	\pause
	\begin{itemize}
		\item J'effectue un lancer avec cette pièce et le résultat est "Pile".
...
\begin{frame}{Un exemple avec une pièce de monnaie}
	\begin{itemize}
		\item Moins la probabilité $P(X = \text{"Pile"})$ est forte, plus la \textbf{surprise} devient grande lorsque que le lancer aboutit sur "Pile".
		\pause
...
\begin{frame}{Surprise de deux événements indépendants}
	Supposons que les probabilités de notre pièce sont maintenant les suivantes:
	\begin{equation*}
		\begin{cases}
			P(X = \text{"Face"}) = 0.2 \\
			P(X = \text{"Pile"}) = 0.8
	...
	\end{equation*}
	\pause
	J'effectue 2 lancers successifs et \textbf{indépendants} et j'obtiens les résultat suivant: $\{\text{"Pile"}, \text{"Face"}\}$. Quelle est la surprise associée à ces lancers ?
	\begin{equation*}
...
\begin{frame}{Surprise de deux événements indépendants}
	...
	\end{equation*}
	\pause
	La surprise jointe de deux d'événements indépendants $A$ et $B$ est la somme des surprises de ces deux événements:
	\begin{equation*}
...
\begin{frame}{Intuition de l'entropie croisée}
	\begin{itemize}
		...
		\pause
		\item Mais il y a une différence entre la probabilité \textbf{réelle} d'occurrence d'un lancer aboutissant au résultat "Pile" et la probabilité que vous avez utilisée pour calculer la surprise qui correspond à la probabilité que je vous ai \textbf{communiquée}.
		\pause
...
\begin{frame}{La surprise pour une pièce spécifique}
	...
	\pause
	\begin{itemize}
		\item Puisque le résultat est connu et certain, la probabilité \textbf{empirique} $\hat{P}$ pour cette pièce est exactement égale à son étiquette $y^{(i)}$.
		\item Le modèle, lui, émet une probabilité \textbf{estimée} $Q$ que l'on note $q^{(i)}$.
	\end{itemize}
	\pause
	\vspace{0.3cm}
	On peut donc écrire la surprise du modèle pour cette pièce en remplaçant $\hat{P}$ par $y^{(i)}$ et $Q$ par $q^{(i)}$ :
	\begin{equation*}
...
\begin{frame}{La fonction de perte}
	...
	\begin{equation*}
		H(\hat{P}, Q) 
		= - \frac{1}{n} \sum_{i=1}^n \left[ y^{(i)} \ln{\left(q^{(i)}\right)} + (1 - y^{(i)}) \ln{\left(1 - q^{(i)}\right)} \right]
	\end{equation*}
	\pause
